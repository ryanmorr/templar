...
describe('templar', function () {
    it('should support node interpolation with a string', function () {
        var tpl = (0, _templar2.default)('<div>{{str}}</div>');
        tpl.set('str', 'foo');
        (0, _chai.expect)(tpl.frag.childNodes[0].textContent).to.equal('foo');
    });

    it('should support node interpolation with a number', function () {
        var tpl = (0, _templar2.default)('<div>{{num}}</div>');
        tpl.set('num', 25);
        (0, _chai.expect)(tpl.frag.childNodes[0].textContent).to.equal('25');
    });

    it('should support node interpolation with a boolean', function () {
        var tpl = (0, _templar2.default)('<div>{{bool}}</div>');
        tpl.set('bool', true);
        (0, _chai.expect)(tpl.frag.childNodes[0].textContent).to.equal('true');
    });
